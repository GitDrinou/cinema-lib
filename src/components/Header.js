...
export const Header = () => {
  ...
  const activeClass = "text-sm block py-2 px-3 text-white bg-cyan-700 font-extrabold rounded md:bg-transparent en md:p-0 md:dark:text-cyan-500";
  const inactiveClass = "text-sm block py-2 px-3 text-gray-900 rounded hover:bg-gray-100 md:hover:bg-transparent md:hover:text-cyan-700 md:p-0 dark:text-white md:dark:hover:text-cyan-500 dark:hover:bg-gray-700 dark:hover:text-white md:dark:hover:bg-transparent dark:border-gray-700";
  ...
  const handleSubmit = (event) => {
    ...
    return navigate(`/search?query=${queryString}`);
  }
      ...
      <nav className="bg-white border-gray-200 dark:bg-gray-900">
        <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <Link to="/" className="flex items-center space-x-3 rtl:space-x-reverse">
              <img src={Logo} className="h-8" alt="CineLib Logo" />
              <span className="self-center text-2xl font-semibold whitespace-nowrap dark:text-white">CineLib</span>
          </Link>
          ...
          <div id="nav-links" className={`items-center ${hidden ? "hidden" : ""} justify-between w-full md:flex md:w-auto md:order-1`}>
            <div className="relative mt-3 md:hidden">
              ...
              <form onSubmit={handleSubmit}>
                <input type="text" id="search-navbar" name="search" className="block w-full p-2 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search..." autoCapitalize="off"/>
              </form>
            </div>
